import { registerSWRRoutes, cleanupSWRServices } from '../../../src/api/routes/swr.routes';
import { z } from 'zod';
...
jest.mock('../../../src/services/queue/backgroundJobs', () => ({
...
describe('SWR API Routes', () => {
  ...
  describe('GET /api/v1/weights', () => {
    ...
    it('should handle errors gracefully', async () => {
      const { RedisWeightRegistry } = require('../../../src/services/swr/redisClient');
  ...
  describe('POST /api/v1/weights/pull', () => {
    ...
    it('should handle queue errors', async () => {
      const { BackgroundJobQueue } = require('../../../src/services/queue/backgroundJobs');
  ...
  describe('Performance and Timing', () => {
    it('should measure timing for list endpoint', async () => {
      await registerSWRRoutes(mockFastify);
      ...
      const sentResponse = mockReply.send.mock.calls[0][0];
      const timingMs = parseInt(sentResponse.timing.replace('ms', ''));
      expect(timingMs).toBeGreaterThanOrEqual(0);
    });

    it('should measure timing for get endpoint', async () => {
      await registerSWRRoutes(mockFastify);
    ...
    it('should measure timing for pull endpoint', async () => {
      await registerSWRRoutes(mockFastify);
    ...
    it('should measure timing for stats endpoint', async () => {
      await registerSWRRoutes(mockFastify);
